...
int lambdafuh(int n, int m, const double* a, const double* Q, double* F_, double* s)
{
    ...
    if (!(info = LD(n, Q, L, D)))
    {
        ...
        {
            MatrixInv(n, Z, ZT);
            for (i = 0; i < m; i++) MatrixMultiply(n, n, n, 1, ZT, E + i * n, F_ + i * n);
        }
    }
...
int lambda(int m,Matrix* amb, Matrix* Q_amb, Matrix* F_, Matrix* s)
{
    ...
    memcpy(a, amb->data(),  n * sizeof(double));
    memcpy(Q, Q_amb->data(),  n * n * sizeof(double));
    Matrix F_temp(n, m); (*F_) = F_temp;
    Matrix s_temp(m, 1); (*s) = s_temp;
    double* F_fuh = new double[n  * m];
    double* sfuh=new double[m];
    lambdafuh(n,m,a,Q, F_fuh, sfuh);
    (*F_) = (*F_).transpose();
    (*s) = (*s).transpose();
    memcpy(F_->data(), F_fuh, n * m * sizeof(double));
    memcpy(s->data(), sfuh,  m* sizeof(double));
    (*F_) = (*F_).transpose();
    (*s) = (*s).transpose();
    return 1;
}
